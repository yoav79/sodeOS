...
export default function EditorAgentTab({
  ...
  return (
    <div className="h-full flex flex-col min-h-0 text-slate-700">
      {/* Zona 1: Área superior scrolleable */}
      ...
      {/* Zona 2: Formulario fijo inferior */}
      <div className="shrink-0 border-t border-slate-200/80 bg-white p-4 space-y-3 shadow-md">
        {/* Input de consulta */}
        ...
        <div className="space-y-1.5">
          ...
          {isOutputModeHelpOpen && (
            <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-2.5 text-[10.5px] leading-relaxed text-slate-600 max-h-48 overflow-y-auto">
          ...
          <>
            {agentLoadingStage === 'done' ? (
              <button
                type="button"
                onClick={handleReset}
                className="w-full py-2 px-3 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 rounded-lg text-xs font-semibold shadow-xs transition-colors flex items-center justify-center gap-1.5"
              >
