        ...
        {localBrains.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {localBrains.map((brain) => (
              <div
                key={brain.id}
                className="bg-gradient-to-br from-white via-white to-slate-50 border border-slate-200/80 rounded-3xl shadow-sm hover:shadow-md hover:border-blue-200 hover:shadow-blue-900/5 transition-all duration-300 p-6 flex flex-col justify-between"
              >
                <div>
                  {/* 1. Header principal */}
                  <div className="flex gap-4 items-start relative mb-4">
                    {/* Left: Large visual brain icon */}
                    ...
                    {/* Right/Middle: Title and Description */}
                    <div className="min-w-0 flex-1 pr-6">
                      <h3 className="font-bold text-slate-900 text-lg md:text-xl leading-tight mb-1 truncate" title={brain.name}>
                        {brain.name}
                      </h3>
                  ...
                  {/* 3. Acceso principal de navegación */}
                  <div className="mb-4">
                    <Button
                      onClick={() => router.push(`/brains/${brain.id}`)}
                      variant="primary"
                      className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm rounded-xl transition-all flex items-center justify-center gap-2 shadow-sm shadow-blue-500/20 hover:shadow-md hover:shadow-blue-500/25 border-none"
                      rightIcon={
                ...
                {/* 5. Footer metadata */}
                <div className="flex justify-between items-center text-xs text-slate-500 gap-4 mt-auto">
                  {/* Left side: Nodes */}
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col text-left">
                      <span className="font-bold text-slate-700 leading-tight text-xs">
                        {brain.activeNodesCount} {brain.activeNodesCount === 1 ? 'nodo' : 'nodos'}
                      </span>
                      <span className="text-[10px] text-slate-400">Contenido del cerebro</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-right">
                    <div className="flex flex-col">
                      <span className="font-bold text-slate-700 leading-tight text-xs">Actualizado</span>
                      <span className="text-[10px] text-slate-400">{formatDate(brain.updatedAt)}</span>
                    </div>
                    <div className="p-1.5 bg-slate-50 border border-slate-100 rounded-lg text-slate-400">
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2-2v-6a2 2 0 002-2z" />
                      </svg>
                    </div>
                  </div>
                </div>
